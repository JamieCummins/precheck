...
def test_enabled_blocks_after_limit_per_ip(limiter_state):
    ...
    a = [c.get("/go", headers={"x-forwarded-for": "spoofed, 1.1.1.1"}).status_code for _ in range(4)]
    assert a == [200, 200, 200, 429]
    # A different real client IP (the trailing, router-appended entry) is
    # unaffected by A's usage.
    assert c.get("/go", headers={"x-forwarded-for": "2.2.2.2"}).status_code == 200


def test_client_key_uses_last_forwarded_for_entry():
    """Heroku appends the real client IP to X-Forwarded-For; every earlier entry
    is client-supplied. Keying on the last entry means spoofed prefixes cannot
    mint fresh rate-limit buckets."""
    ...
    assert rl._client_key(req("203.0.113.5, 10.0.0.1")) == "10.0.0.1"
    # Varying the client-supplied prefix must not change the key.
    assert rl._client_key(req("a, 10.0.0.1")) == rl._client_key(req("b, 10.0.0.1"))
