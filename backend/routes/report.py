...
async def report_export_html(request: Request, task_id: str):
    ...
    document = build_standalone_report_html(
        ...
        meta=f"Exported from PreCheck on {date.today().isoformat()}",
    )
    filename = "precheck-report.html"
    headers = {
        **PRIVATE_HEADERS,
...
# Rendered page PNGs are cached in Redis so repeat views of the same report
# don't re-rasterize on every request (rasterization is CPU-bound and this
# endpoint is reachable by anyone with a report link). Keyed per task so cache
# entries die with their report; TTL keeps Redis growth bounded.
_PAGE_PNG_CACHE_TTL_SECONDS = 3600
...
async def report_source_page_png(request: Request, task_id: str, source_id: str, page_number: int):
    ...
    await _ensure_access(request, task_id)
    redis_client = request.app.state.redis
    cache_key = f"pagepng:{task_id}:{source_id}:{page_number}"
    try:
        cached = await redis_client.get(cache_key)
    except Exception:  # pragma: no cover - cache is best-effort
        cached = None
    if cached:
        return Response(content=cached, media_type="image/png", headers=PRIVATE_HEADERS)
    ...
    try:
        await redis_client.set(cache_key, png, ex=_PAGE_PNG_CACHE_TTL_SECONDS)
    except Exception:  # pragma: no cover - cache is best-effort
        pass
