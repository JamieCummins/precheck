"""Command-line entrypoint to run PreCheck assessments and comparisons without the frontend.
...
def _maybe_write_html_report(args, payload: dict, evidence_out: dict | None) -> None:
    ...
    title = getattr(args, "report_title", None) or f"PreCheck · {Path(args.paper).stem}"
...
async def _run_quality(args) -> dict:
    ...
    from backend.services import comparisons as comparisons_service
    from backend.services.dimensions import registration_quality_dimensions
    from backend.services.llm import resolve_judge_model
    ...
    custom = bool(dimensions)
    criteria_set = getattr(args, "criteria_set", None)
    if not custom and criteria_set:
        dimensions = registration_quality_dimensions(criteria_set)
    prereg_path, prereg_ext = _resolve_general_prereg(args)
    ...
    if prompt_file:
        ...
        custom_doctrine = prompt_path.read_text(encoding="utf-8")
        if not custom_doctrine.strip():
            raise ValueError(f"Prompt file is empty: {prompt_file}")
        logger.warning(
            "Using a CUSTOM master prompt from %s — this bypasses the signed-off doctrine wording.",
            prompt_file,
        )

    use_rag = _resolve_use_rag(args)
    logger.info(
        "Running registration quality assessment (dimensions=%s, prompt=%s, retrieval=%s)",
        "custom" if custom else (criteria_set or "default"),
        "custom" if custom_doctrine else "default",
        "rag" if use_rag else "full_document",
    )
    ...
    result = await registration_quality_assessment(
        ...
        append_previous_output=args.append_previous_output,
        reasoning_effort=args.reasoning_effort,
        custom_doctrine=custom_doctrine,
        retrieval_context_out=retrieval_context,
        use_rag=use_rag,
    )
    logger.info("Completed registration quality assessment.")

    payload = result.model_dump()
    # Run-level metadata for the analysis-friendly output table (see
    # _write_output's quality branch): these repeat on every per-dimension row.
    payload["quality_meta"] = {
        "model_name": resolve_judge_model(args.client),
        "prompt_type": "custom" if custom_doctrine else "default",
        # per_dimension = one dimension at a time, blind to its own other
        # verdicts; --append-previous-output un-blinds it, recorded honestly.
        "prompt_focus": (
            "per_dimension_with_history" if args.append_previous_output else "per_dimension"
        ),
        "retrieval": "rag" if use_rag else "full_document",
        "chunk_size": comparisons_service._embedding_max_chunk_tokens(),
        "price_in_usd": (payload.get("cost") or {}).get("total_usd"),
    }
    # Attach the full judge context per item ("text" column). Keyed by the
    # dimension name we asked for; fall back to run order (one item per
    # dimension) when the judge echoed a differently-normalised name.
    context_values = list(retrieval_context.values())
    for index, item in enumerate(payload.get("items", [])):
        if isinstance(item, dict):
            fallback = context_values[index] if index < len(context_values) else ""
            item["text"] = retrieval_context.get((item.get("dimension") or "").strip(), fallback)
    return payload
...
def _batch_row_namespace(args, row: dict[str, str]) -> argparse.Namespace:
    ...
    row_no_rag = (get("no_rag") or "").lower() in {"1", "true", "yes", "on"}
    return argparse.Namespace(
        ...
        criteria_set=get("criteria_set") or "psychology",
        prompt_file=get("prompt_file"),
        client=args.client,
        parser_choice=args.parser_choice,
        append_previous_output=args.append_previous_output,
        reasoning_effort=args.reasoning_effort,
        no_rag=row_no_rag or getattr(args, "no_rag", False),
...
async def _run_batch(args) -> dict:
    ...
    ``registration_id``, optional ``dimensions_csv``/``dimension_set``,
    ``criteria_set`` (quality: psychology|clinical), ``prompt_file`` (quality:
    custom master prompt), ``no_rag`` (quality: 1/true/yes = full-document mode
    for that row) and ``label``. Per-row results are written to
    --output-dir; failures are recorded and the batch continues (use
    --stop-on-error to abort instead)."""
...
def _write_output(payload: dict, output_path: str | None, output_format: str) -> None:
    ...
    # CSV output (default)
    items = payload.get("items", []) if isinstance(payload, dict) else []
    quality_meta = payload.get("quality_meta") if isinstance(payload, dict) else None
    if quality_meta is not None:
        # Quality runs use the analysis-table shape: run-level columns
        # (model_name … price_in_usd) repeat on every per-dimension row.
        from backend.services.registration_quality import normalize_quality_verdict

        # PreCheck's decision criteria (maintainer wording, 2026-08-21) keyed by
        # the internal wire tokens.
        evaluation_labels = {
            "complete": "present and no revision required",
            "partial": "present and revision required",
            "absent": "not present",
        }
        ...
        run_level = {
            key: ("" if quality_meta.get(key) is None else str(quality_meta.get(key)))
            for key in ("model_name", "prompt_type", "prompt_focus", "retrieval", "chunk_size", "price_in_usd")
        }
        rows = []
        for item in items:
            ...
            rows.append(
                {
                    **run_level,
                    ...
                    "evaluation": evaluation_labels[normalize_quality_verdict(item.get("deviation_judgement"))],
                    "justification": str(item.get("deviation_information") or ""),
                    "registration_content_quotes": str(item.get("registration_content_quotes") or ""),
                    "unlocated_in_registration": str(item.get("unlocated_in_registration") or ""),
                }
            )
    else:
        fieldnames = [
...
def _resolve_use_rag(args) -> bool:
    """Resolve retrieval on/off for the quality flow.

    ``--no-rag`` always wins; otherwise the QUALITY_USE_RAG env var sets the
    default (unset/1/true = RAG on). RAG off = the judge sees the whole
    registration on every dimension call and no embedding calls are made.
    """
    if getattr(args, "no_rag", False):
        return False
    return (os.environ.get("QUALITY_USE_RAG") or "1").strip().lower() not in {"0", "false", "no", "off"}
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run PreCheck backend comparisons from the command line."
    )
    ...
    general.add_argument(
        "--report-title",
        help="Title shown in the --report-html report (defaults to 'PreCheck · <paper filename>').",
    )
    ...
    batch.add_argument(
        ...
        action="store_true",
        help=(
            "Quality rows only: disable retrieval, showing the judge the whole "
            "registration per dimension. A per-row 'no_rag' manifest column "
            "(1/true/yes) overrides per row."
        ),
    )
    ...
    quality.add_argument(
        "--criteria-set",
        default="psychology",
        choices=["psychology", "clinical"],
        help="Which default criteria set to assess against when no --dimensions-csv is given.",
    )
    quality.add_argument(
        "--prompt-file",
        help=(
            "Path to a text file replacing the default master prompt (the static doctrine). "
            "Outputs record prompt_type=custom. NOTE: bypasses the signed-off prompt wording."
        ),
    )
    quality.add_argument(
        "--client",
    ...
    quality.add_argument(
        ...
        action="store_true",
        help=(
            "Disable retrieval: the judge sees the WHOLE registration (all chunks, "
            "document order) on every dimension call; no embedding calls, no targeted "
            "verification loop. Default comes from QUALITY_USE_RAG (unset/1 = RAG on)."
        ),
    )
    quality.add_argument(
        "--append-previous-output",
