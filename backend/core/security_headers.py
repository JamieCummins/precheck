...
# everything is self-hosted (fonts are vendored under static/vendor/fonts, so no
# Google Fonts / CDN origins remain). Inline <script>/<style> exist on a few
# pages, so script/style allow 'unsafe-inline' (no nonces with static templates)
# — but only 'self' + inline, never external script origins. blob:/data: cover
# the report viewer's CSV download + rendered pages. No cross-origin form
# actions, so form-action stays 'self'. frame-ancestors/object-src/base-uri
# close the common clickjacking / plugin / base-injection holes. Only applied to
# HTML documents (static assets and JSON API responses don't need it).
DEFAULT_CSP = "; ".join(
    [
        "default-src 'self'",
