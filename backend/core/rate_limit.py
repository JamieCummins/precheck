...
def _client_key(request: Request) -> str:
    ...
    Behind Heroku's router the socket peer is the router, not the user. Heroku
    APPENDS the connection's real client IP to any ``X-Forwarded-For`` the
    client sent, so the only trustworthy entry is the LAST one — a client can
    fully control the left-most entries and would otherwise mint a fresh
    rate-limit bucket per request. Falls back to the direct peer address
    off-platform.
    """
