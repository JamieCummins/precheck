...
STATIC_VERSION = "20260821-verdict-labels"
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="PreCheck",
        version=APP_VERSION,
        description="Evaluate how completely a study registration specifies the planned study.",
        lifespan=lifespan,
        docs_url=None,
        redoc_url=None,
    )
    ...
    # Single-brand deployment: this codebase IS PreCheck.
    app.state.templates.env.globals["app_mode"] = "precheck"
    app.state.templates.env.globals["brand_name"] = "PreCheck"
    # Footer copyright year; refreshed on process start, which is plenty on
    # Heroku's daily dyno cycling.
    app.state.templates.env.globals["current_year"] = datetime.now(timezone.utc).year
