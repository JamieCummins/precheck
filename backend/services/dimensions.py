...
REGISTRATION_QUALITY_DEFAULT_SET = "psychology"


def _registration_quality_sets() -> dict:
    entry = _discipline_data().get("registration_quality") or {}
    return entry.get("sets") or {}


def registration_quality_dimensions(set_key: str | None = None) -> list[dict[str, str]]:
    ...
    comparison wizard's discipline picker). Field-specific default sets live under
    registration_quality.sets; psychology is the default."""
    sets = _registration_quality_sets()
    chosen = sets.get(set_key or REGISTRATION_QUALITY_DEFAULT_SET) or sets.get(
        REGISTRATION_QUALITY_DEFAULT_SET
    ) or {}
    return [dict(d) for d in chosen.get("dimensions", [])]


def registration_quality_set_for_ui() -> list[dict]:
    """The quality criteria sets shaped like discipline_sets_for_ui() output, so the
    quality wizard reuses the same dimension editor and preset picker. The default
    set (psychology) is listed first."""
    sets = _registration_quality_sets()
    ordered = [REGISTRATION_QUALITY_DEFAULT_SET] + [
        k for k in sets if k != REGISTRATION_QUALITY_DEFAULT_SET
    ]
    return [
        {
            "key": key,
            "label": sets[key].get("label", key.title()),
            "dims": [
                {"name": d.get("dimension", ""), "definition": d.get("definition", "")}
                for d in sets[key].get("dimensions", [])
            ],
        }
        for key in ordered
        if key in sets
