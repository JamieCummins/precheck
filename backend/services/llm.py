...
def resolve_judge_model(client_choice: str) -> str:
    """The concrete model name a given --client choice resolves to (for run
    metadata / output tables; mirrors the resolution the dispatch paths use)."""
    choice = (client_choice or "").strip().lower()
    if choice == "deepseek":
