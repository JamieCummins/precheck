(function () {
    ...
    function judgementInfo(value) {
        ...
        if (isQualityReport()) {
            if (normalized === "complete") return { label: "Present and no revision required", tone: "ok", desc: "This criterion is specified precisely enough that a deviation could later be detected — no revision needed." };
            if (normalized === "partial") return { label: "Present and revision required", tone: "warn", desc: "This criterion is addressed, but at least one element is missing, vague, or ambiguous — revise before running the study." };
            if (normalized === "absent") return { label: "Not present", tone: "flag", desc: "The registration does not usably address this criterion." };
    ...
    function downloadCsv() {
        const quality = isQualityReport();
        const rows = quality
            ? [["Dimension", "Registration Direct Quotes", "Evaluation", "Justification", "Chain Of Thought"]]
            : [[
                "Dimension",
        ...
        state.items.forEach((item) => {
            if (quality) {
                // Quality mode speaks the decision criteria, not the wire tokens.
                rows.push([
                    item.dimension || "",
                    quotesPlain(item.registration_content_quotes),
                    judgementInfo(item.deviation_judgement).label.toLowerCase(),
                    item.deviation_information || "",
        ...
        link.download = "precheck-report.csv";
